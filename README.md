...
> **Development branch:** `dev/tou-modbus-research` carries version `1.1.0b6` with explicit Read only and Read/write installation modes. Read only is the safe default. Read/write exposes the validated master-only TOU editor; generator controls; and open-loop fallback absorption/float settings. It is a test build, not the stable 1.0 release. Numeric controls use direct-entry boxes rather than sliders. During normal closed-loop operation, the BMS may overwrite the fallback voltage values.
